...
class Events(object):
    ...
    def get_all(self, start=0, count=-1, _filter='', query='', sort='', view=''):
        """
        Gets all the events based upon filters provided.

        Args:
            start:
                 The first item to return, using 0-based indexing. If not specified,
            ...
            sort:
                ...
                based on create time, with the oldest entry first.
            view:
                 Returns a specific subset of the attributes of the resource or collection,
                 by specifying the name of a predefined view. The default view is expand
        ...
        """
        return self._client.get_all(start=start, count=count, filter=_filter, query=query, \
                sort=sort, view=view)
